...
async def cron(conn, cursor, client):
    now = datetime.now().strftime("%H:%M")
    logger.debug(f"[cron] Current time: {now}")
    ...
    if now == "00:00":
        try:
            ...
            logger.info(f"[cron] Retrieved {len(user_ids)} user(s) for progress check")
            ...
            async def send_progress(user_id):
                try:
                    ...
                    if not progress:
                        logger.info(f"[cron] No progress to send for user {user_id}")
                        return
    ...
    try:
        ...
        logger.debug(f"[cron] Fetched {len(users)} users for notice check")
